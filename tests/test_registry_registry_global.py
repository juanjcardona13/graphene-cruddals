...
def test_register_model(registry):
    ...
    assert registry.get_registry_for_model(model) == {"type_a": "value_a"}, (
...
def test_get_all_models(reset_registry, registry):
    ...
    assert len(registry.get_all_models_registered()) == 2, (
...
def test_get_all_fields():
    ...
    assert len(custom_registry.get_all_fields_registered()) == 2, (
...
def test_hashable_value_conversions():
    ...
    assert isinstance(registry.get_hashable_value({"key": "value"}), tuple), (
        "Dictionary not converted to tuple"
    )
    assert isinstance(registry.get_hashable_value(["item1", "item2"]), tuple), (
...
def test_global_registry_creation(registry):
    ...
    assert isinstance(reg, RegistryGlobal), (
